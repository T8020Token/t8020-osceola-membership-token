...
class DynamicApiClient extends BaseClient {
    ...
    async storeEncryptedBackupByWallet({ walletId, encryptedKeyShares, passwordEncrypted, signedSessionId, encryptionVersion, requiresSignedSessionId = true, authMode = AuthMode.HEADER }) {
    ...
    async recoverEncryptedBackupByWallet({ walletId, keyShareIds, signedSessionId, mfaToken, requiresSignedSessionId = true, authMode = AuthMode.HEADER }) {
        if (requiresSignedSessionId && !signedSessionId) {
            throw new Error('Signed session ID is required');
        }
        // When using cookie-based authentication, use the main apiClient instead of clientRelayApiClient.
