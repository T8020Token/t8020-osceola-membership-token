...
const DefaultTile = ({ leading, name, onClick, trailing, }) => (jsxRuntime.jsx(ListTile.ListTile, { leading: leading, trailing: trailing, onClick: onClick, className: 'exchange-list-item__tile', dataTestId: 'ListTile', children: name }));
const ExchangeListItem = ({ disabled = false, exchange, onResetSearchValue, tile = DefaultTile, onSelectExchange, trailingProp = undefined, }) => {
    ...
    const name = exchange;
    const connected = (_b = (_a = user === null || user === void 0 ? void 0 : user.verifiedCredentials) === null || _a === void 0 ? void 0 : _a.some(({ oauthProvider }) => oauthProvider === convertExchangeKeyAndProviderEnum.convertExchangeKeytoProviderEnum(exchange))) !== null && _b !== void 0 ? _b : false;
    const leading = (jsxRuntime.jsx(iconic.SocialIcon, { name: convertExchangeKeyAndProviderEnum.convertExchangeKeytoProviderEnum(exchange), variant: iconThemeVariant, style: { height: pixelToRem.pixelToRem(28), width: pixelToRem.pixelToRem(28) } }));
...
exports.ExchangeListItem = ExchangeListItem;
